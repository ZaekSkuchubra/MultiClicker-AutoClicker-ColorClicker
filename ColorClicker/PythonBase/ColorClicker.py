...
def auto_click_until_color_change():
    ...
    initial_color = get_pixel_color(initial_position.x, initial_position.y)
    print(f"Current color: {initial_color}")
    try:
        while True:
            current_position = pyautogui.position()
            current_color = get_pixel_color(current_position.x, current_position.y)
            if current_color != initial_color:
                print(f"Color around mouse changed to: {current_color}")
                break
            pyautogui.click()
